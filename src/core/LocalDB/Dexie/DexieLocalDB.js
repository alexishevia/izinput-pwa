...
function ByName(name) {
  ...
  function getExistingTransaction(id) {
    return getTransaction(id).then((transaction) => {
      if (!transaction) {
        throw new ErrNoExistingTransaction(id);
      }
      return transaction;
    });
  }
  ...
  function checkNoExistingTransaction(id) {
    return getTransaction(id)
      .then(Boolean)
      .then((exists) => {
        if (exists) {
          throw new ErrExistingTransaction(id);
        }
      });
  }
  ...
  function createTransaction({ payload }) {
    return Promise.resolve()
      ...
      .then(() => checkNoDelete(payload))
      .then(() => checkNoExistingTransaction(payload.id))
  ...
  function updateTransaction({ payload }) {
    return Promise.resolve()
      .then(() => checkNoDelete(payload))
      .then(() => getExistingTransaction(payload.id))
      .then((existing) => {
        checkNoUpdateConflict(existing, payload);
        return existing;
      })
