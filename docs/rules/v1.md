...
A payment represents a deposit of money into an account.

Payments have the following fields:
...
|                 | account ID    | payment is being deposited into. |                                        |
|                 |               |                                  |                                        |
| categoryID      | string        | ID of the category associated    | "49f4d831-feb2-498b-8079-1c5b61dc9301" |
|                 | category ID   | with this payment.               |                                        |
|                 |               |                                  |                                        |
| description     | string        | Additional notes for the         | "Freelance Project X"                  |
|                 |               | payment.                         |                                        |
|                 |               |                                  |                                        |
| transactionDate | string        | Date the payment happened.       | "2019-10-12"                           |
|                 | YYYY-MM-DD    |                                  |                                        |
|                 |               |                                  |                                        |
| modifiedAt      | string        | Last date the payment was        | "2019-10-12T12:25:35.059Z"             |
|                 | full ISO 8601 | modified. (create, update, and   |                                        |
|                 | date in UTC   | delete count for "modifiedAt")   |                                        |
|                 |               |                                  |                                        |
| deleted         | boolean       | true if the payment has been     | false                                  |
|                 |               | deleted. false otherwise.        |                                        |
...
Use this action to create a payment.
...
1. IF the payment `id` is unique (no other payments with the same `id` exist)  
    AND all payment fields are included in `payload`  
    AND all payment field values have the correct type  
    AND the `payload.accountID` account exists  
    AND the `payload.categoryID` category exists  
    AND `payload.deleted` is `false`  
   THEN a payment MUST be created using the `payload` values.

Notes:

1. While not required, it is recommended to use UUIDs for payment ids.
   Using UUIDs diminishes the probability of unwanted conflicts.

### v1 payments/update

Use this action to update a payment

Note: to delete a payment, use `payments/delete` instead.
...
1. IF the payment `id` is not unique (another payment with the same `id` already exists)  
    AND a valid `payload.modifiedAt` value is included  
    AND `payload.modifiedAt` is later than the existing payment's `modifiedAt`  
    AND `payload.deleted` is NOT `false`  
   THEN the data in `action.payload` must be merged with the existing payment's data.  
    ie:  
    fields included in `action.payload` override the same fields in the existing payment  
    fields in the existing payment that are NOT included in `action.payload` remain unchanged

### v1 payments/delete

Use this action to delete an existing payment.
...
1. IF the payment `id` is not unique (a payment with the same `id` already exists)  
    AND a valid `payload.modifiedAt` value is included  
    AND `payload.modifiedAt` is later than the existing payment's `modifiedAt`  
   THEN the payment's `deleted` field must be set to `true`
   THEN the payment's `modifiedAt` must be set to `payload.modifiedAt`
